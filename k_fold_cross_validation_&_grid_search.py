...
classifier_without_gender = SVC(kernel='rbf')
classifier_with_gender = SVC(kernel='rbf')
...
accuracy = cross_val_score(classifier_with_gender, X, Y, cv=10)
accuracy.mean()
accuracy.std()

# parameter tuning
parameters = [{'C': [0.5, 1, 10], 'kernel':['linear'], 'degree':[1, 2, 3, 4, 5, 6]},
              {'C': [0.5, 1, 10], 'kernel':['rbf'], 'gamma':[.1, .2, .3, .4, .5, .6, .7, .8]}]
grid_search = GridSearchCV(estimator=classifier_with_gender, param_grid=parameters, scoring='accuracy',cv=10,n_jobs=-1 )
grid_search.fit(X, Y)
